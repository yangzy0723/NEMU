...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(dest) = imm);
	...
	INSTPAT("??????? ????? ????? 001 ????? 11000 11", neq    , B, s->dnpc = (src1 != src2) ? imm + s->pc : s->snpc);
	/*I*/
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = Mr(src1 + imm, 4));
	...
	INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = src1 >> BITS(imm, 5, 0));//may be wrong
	INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = 0x0000ffff&Mr(src1 + imm, 4));
	INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = imm & src1);
	INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(dest) = imm ^ src1);
	/*S*/
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
	INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 4, src2&0x0000ffff));
	INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 4, src2&0x000000ff));
	/*R*/
