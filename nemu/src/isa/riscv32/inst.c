...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
	...
	INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(dest) = src1 << (src2&0x0000001f));
	...
	INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, tmp = (int64_t)((sword_t)src1)*(int64_t)((sword_t)src2), R(dest) = BITS(tmp, 63, 32));//要先转成有符号数字相乘，因为是有符号乘。
