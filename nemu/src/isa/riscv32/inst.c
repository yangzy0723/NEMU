...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(dest) = imm);
	...
	INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = (src1 >= src2) ? imm + s->pc : s->snpc);
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = Mr(src1 + imm, 4));
	...
	INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (sword_t)src1 >> BITS(imm, 5, 0));//may be wrong
	...
	INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(dest) = BITS(Mr(src1 + imm, 4), 15, 0));
	INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 << imm);
	INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = (word_t)src1 >> BITS(imm, 5, 0));
